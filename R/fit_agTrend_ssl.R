...
fit_agTrend_ssl <- function(
  .x, model, obl.corr=FALSE, penalty=TRUE,map.override=NULL, 
  ...
  if(model=="gp"){
    ...
    K <- iar_basis(nrow(.x), 2, nrow(.x))
    K <- K-P%*%K
    Kaug <- rbind(K, 10*diag(ncol(K)))
  }
